...
void print_stats(RepetitionTester *tester) {
  ...
  u64 frequency = get_cpu_timer_frequency();

  printf("THROUGHPUT: %.2fGb/s\n", (f64)tester->bytes_processed / 1024. / 1024. / 1024. / (tester->avg_time / (f64)frequency) / tester->num_repetitions);
}
...
static void handle_allocation(ReadParameters *Params,
                              buffer *Buffer) {
