...
f64 fit_taylor_horner(size_t order, f64 x, f64* coeffs) {
  ...
  for(int i = 1; i <= real_order; i+=2) {
    ...
    vy = vfma_f64(vdup_n_f64(coeffs[pow/2]), vy, x_2);
  }
...
f64 csin(f64 v) {
  ...
  f64 res;
  if (fabs(v) > PI/2) {
    res = fit_taylor_horner(30, PI/2 - (fabs(v) - PI/2), sin_coeffs);
  } else {
    res = fit_taylor_horner(30, fabs(v), sin_coeffs);
  }
...
f64 casin(f64 v) {
  f64 res;
  if (fabs(v) > 1/csqrt(2)) {
    res = PI/2 - fit_taylor_horner(57, csqrt(1 - v*v), arcsin_coeffs);
  } else {
    res = fit_taylor_horner(57, fabs(v), arcsin_coeffs);
  }
